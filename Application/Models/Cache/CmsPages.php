 ...
 * CMSc_Cache_CmsPages
 */
abstract class CmsPages extends Cache
{
        ...
        if ( !static::$_oInstance ) {
            ...
            if ( $sEngine === CMSc_Utils::VALUE_CMS_PAGES_CACHE_ENGINE_AUTO ) {
                if ( extension_loaded('memcached') ) {
                    static::$_oInstance = new Cache\CmsPages\memcached();
                } else if ( extension_loaded('memcache') ) {
                    static::$_oInstance = new Cache\CmsPages\memcache();
                } else {
                    static::$_oInstance = new Cache\CmsPages\OxidFileCache();
                }
            } else {
                switch ( $sEngine ) {
...
//        return 'CMSc_CmsPage_';
    ...
    public function saveHttpResult ($oCmsPage, $oHttpResult, $iTtl = null)
    {
        ...
        $oHttpResult->ttl = Registry::get('oxUtilsDate')->getTime() + $iTtl;
        ...
        if ($blSuccess) {
            $this->_storePageIndexEntry($oCmsPage);
        }
    ...
    public function fetchHttpResultByCacheKey ($sCacheKey)
    {
        ...
        if ( !$oResult  ) {
            ...
            if ( $oResult->ttl <= Registry::get('oxUtilsDate')->getTime() ) {
                $oResult = false;
            }
        }
